...
static int tsc1641_set_shunt(struct tsc1641_data *data, u32 val)
{
    ...
    data->current_lsb_ua = DIV_ROUND_CLOSEST(TSC1641_VSHUNT_LSB_NVOLT * 1000, data->rshunt_uohm);
...
static u16 tsc1641_upd_interval_to_reg(long interval)
{
    ...
    int conv_bits = find_closest(conv, tsc1641_conv_times, ARRAY_SIZE(tsc1641_conv_times));
...
static int tsc1641_chip_write(struct device *dev, u32 attr, long val)
{
    ...
    case hwmon_chip_update_interval:
        return regmap_update_bits(data->regmap, TSC1641_CONFIG, TSC1641_CONV_TIME_MASK,
...
static int tsc1641_temp_write(struct device *dev, u32 attr, long val)
{
    ...
    case hwmon_temp_max:
        /* Clamp to full register range */
        val = clamp_val(val, TSC1641_TEMP_LSB_MDEGC * SHRT_MIN, TSC1641_TEMP_LSB_MDEGC * SHRT_MAX);
...
static umode_t tsc1641_is_visible(const void *data, enum hwmon_sensor_types type, u32 attr, int channel)
{
    switch (type) {
    case hwmon_chip:
        switch (attr) {
...
static int tsc1641_read(struct device *dev, enum hwmon_sensor_types type, u32 attr, int channel, long *val)
{
    switch (type) {
    case hwmon_chip:
...
static int tsc1641_write(struct device *dev, enum hwmon_sensor_types type, u32 attr, int channel, long val)
{
    switch (type) {
    case hwmon_chip:
...
static const struct hwmon_channel_info *const tsc1641_info[] = {
    ...
    HWMON_CHANNEL_INFO(in, HWMON_I_INPUT | HWMON_I_MAX | HWMON_I_MAX_ALARM | HWMON_I_MIN | HWMON_I_MIN_ALARM),
    HWMON_CHANNEL_INFO(curr, HWMON_C_INPUT | HWMON_C_MAX | HWMON_C_MAX_ALARM | HWMON_C_MIN | HWMON_C_MIN_ALARM),
    HWMON_CHANNEL_INFO(power, HWMON_P_INPUT | HWMON_P_MAX | HWMON_P_MAX_ALARM),
    HWMON_CHANNEL_INFO(temp, HWMON_T_INPUT | HWMON_T_MAX | HWMON_T_MAX_ALARM),
    NULL
};

static ssize_t shunt_resistor_show(struct device *dev, struct device_attribute *da, char *buf)
{
    struct tsc1641_data *data = dev_get_drvdata(dev);
...
static ssize_t shunt_resistor_store(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    struct tsc1641_data *data = dev_get_drvdata(dev);
    unsigned int val;
    int ret;
...
static int tsc1641_init(struct device *dev, struct tsc1641_data *data)
{
    ...
    return regmap_write(regmap, TSC1641_MASK, TSC1641_MASK_DEFAULT | FIELD_PREP(TSC1641_ALERT_POL_MASK, active_high));
}

/*
 * Bus-agnostic probe: takes a ready regmap (built from i2c or i3c) and brings
 * up the device + hwmon. Shared by the i2c and i3c probe wrappers below.
 */
static int tsc1641_core_probe(struct device *dev, struct regmap *regmap, const char *name)
{
    ...
    hwmon_dev = devm_hwmon_device_register_with_info(dev, name, data, &tsc1641_chip_info, tsc1641_groups);
    if (IS_ERR(hwmon_dev))
        return PTR_ERR(hwmon_dev);

    dev_info(dev, "power monitor %s (Rshunt = %li uOhm)\n", name, data->rshunt_uohm);

    return 0;
}

static int tsc1641_i2c_probe(struct i2c_client *client)
{
    struct regmap *regmap = devm_regmap_init_i2c(client, &tsc1641_regmap_config);

    if (IS_ERR(regmap))
        return dev_err_probe(&client->dev, PTR_ERR(regmap), "failed to allocate register map\n");

    return tsc1641_core_probe(&client->dev, regmap, client->name);
}

static const struct i2c_device_id tsc1641_id[] = { { "tsc1641", 0 }, {} };
...
static struct i2c_driver tsc1641_i2c_driver = {
	.driver = {
		.name = "tsc1641",
		.of_match_table = of_match_ptr(tsc1641_of_match),
	},
	.probe = tsc1641_i2c_probe,
	.id_table = tsc1641_id,
};

/*
 * Native I3C back-end. On hosts whose Pi-header SDA/SCL are wired to a
 * DesignWare I3C controller (e.g. Agilex 5 HPS I3C0), the TSC1641 enumerates
 * as a native I3C device via DAA rather than legacy-I2C (the DW master cannot
 * mix legacy-I2C with native-I3C devices). The chip is I3C-capable per its
 * datasheet. Match by its MIPI Provisional ID: manufacturer 0x0104 (ST),
 * part 0x020A (read from /sys/bus/i3c/devices/<dev>/pid on hardware). All the
 * register/hwmon logic is shared via tsc1641_core_probe(); only the regmap
 * transport differs.
 */
static int tsc1641_i3c_probe(struct i3c_device *i3cdev)
{
    struct device *dev = i3cdev_to_dev(i3cdev);
    struct regmap *regmap;

#ifdef CONFIG_REGMAP_I3C
    regmap = devm_regmap_init_i3c(i3cdev, &tsc1641_regmap_config);
#else
    regmap = ERR_PTR(-ENODEV);
#endif

    if (IS_ERR(regmap))
        return dev_err_probe(dev, PTR_ERR(regmap), "failed to register i3c regmap\n");

    return tsc1641_core_probe(dev, regmap, "tsc1641");
}

static const struct i3c_device_id tsc1641_i3c_ids[] = { I3C_DEVICE(0x0104, 0x020A, NULL), {} };
MODULE_DEVICE_TABLE(i3c, tsc1641_i3c_ids);

static struct i3c_driver tsc1641_i3c_driver = {
	.driver = {
		.name = "tsc1641_i3c",
	},
	.probe = tsc1641_i3c_probe,
	.id_table = tsc1641_i3c_ids,
};

module_i3c_i2c_driver(tsc1641_i3c_driver, &tsc1641_i2c_driver);
